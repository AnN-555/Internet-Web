document.addEventListener("DOMContentLoaded", function () {
  ...
  if (roomDetail) {
    ...
    const priceTag = roomDetail.querySelector(".room__detail__booking__price p");
  ...
  paymentDetails.forEach((detail) => {
    if (detail.textContent.includes("Deluxe Room") || detail.textContent.includes("Suite")) {
  ...
  if (paymentButton) {
    paymentButton.addEventListener("click", function () {
      localStorage.setItem("stay_location", "Thu Duc Ward, Ho Chi Minh City, Vietnam");
      localStorage.setItem("stay_dates", datesText);

      const roomImg = document.querySelector(".payment__image img") || document.querySelector(".room__image img");
      localStorage.setItem("stay_roomImage", roomImg?.src || "../images/reservation/reservation-page-2/deluxe-room.jpg");

      const roomName = document.querySelector(".room__detail h2")?.textContent.trim();
      localStorage.setItem("stay_roomName", roomName || "Deluxe Room");

      const guestsElement = document.querySelectorAll(".payment__detail")[4];
      localStorage.setItem("stay_guests", guestsElement?.textContent.trim() || "1 Room - 2 Guests");
